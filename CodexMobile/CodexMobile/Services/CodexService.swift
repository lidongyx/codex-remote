...
final class CodexService {
    ...
    // Keeps reconnect fallback on durable relay sessions only; short-lived device targets
    // must be re-resolved instead of being reused from storage.
    var savedRelayReconnectURL: String? {
        guard let sessionId = normalizedRelaySessionId,
              let relayURL = normalizedRelayURL,
              !sessionId.hasPrefix(codexDeviceChannelTokenPrefix) else {
            return nil
        }
    ...
    // Allows one reconnect attempt while iOS background grace is still active for a running turn.
    var canAttemptBackgroundGraceReconnect: Bool {
        !isAppInForeground
