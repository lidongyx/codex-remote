...
async function handleHTTPRequest(req, res, {
  ...
  if (req.method === "POST" && pathname === "/v1/trusted/device/connect") {
    return handleJSONRoute(req, res, async (body) => {
      const result = resolveTrustedDeviceConnection(body);
      const webSocketURL = buildRelayWebSocketURL(req, result.deviceChannelTarget, { trustProxy });
      return {
        ok: result.ok,
        macDeviceId: result.macDeviceId,
        macIdentityPublicKey: result.macIdentityPublicKey,
...
function buildRelayWebSocketURL(req, target, { trustProxy = false } = {}) {
  const normalizedTarget = readHeaderString(target);
  if (!normalizedTarget) {
    return "";
  }

  const host = trustedRequestHost(req, { trustProxy });
  if (!host) {
    return "";
  }

  const protocol = trustedRequestWebSocketProtocol(req, { trustProxy });
  return `${protocol}://${host}/relay/${normalizedTarget}`;
}
...
function trustedRequestHost(req, { trustProxy = false } = {}) {
  if (trustProxy) {
    const forwardedHost = readHeaderString(req?.headers?.["x-forwarded-host"]);
    if (forwardedHost) {
      return forwardedHost.split(",")[0]?.trim() || "";
    }
  }

  return readHeaderString(req?.headers?.host);
}

function trustedRequestWebSocketProtocol(req, { trustProxy = false } = {}) {
  if (trustProxy) {
    const forwardedProto = readHeaderString(req?.headers?.["x-forwarded-proto"]);
    if (forwardedProto) {
      const normalizedForwardedProto = forwardedProto.split(",")[0]?.trim().toLowerCase() || "";
      if (normalizedForwardedProto === "https" || normalizedForwardedProto === "wss") {
        return "wss";
      }
      if (normalizedForwardedProto === "http" || normalizedForwardedProto === "ws") {
        return "ws";
      }
    }
  }
