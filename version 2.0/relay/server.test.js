...
test("trusted device connect returns an opaque device-channel target that routes to the live mac", async () => {
  const phoneIdentity = makePhoneIdentity();

  await withServer(async ({ port }) => {
    const mac = new WebSocket(`ws://127.0.0.1:${port}/relay/live-device-1`, {
      headers: {
        "x-role": "mac",
        "x-mac-device-id": "mac-device-1",
        "x-mac-identity-public-key": "mac-public-key-device-1",
        "x-machine-name": "Device-Mac",
        "x-trusted-phone-device-id": phoneIdentity.phoneDeviceId,
        "x-trusted-phone-public-key": phoneIdentity.phoneIdentityPublicKey,
      },
    });
    ...
    const response = await fetch(`http://127.0.0.1:${port}/v1/trusted/device/connect`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(makeTrustedResolveBody({
        macDeviceId: "mac-device-1",
        phoneIdentity,
        nonce: "nonce-device-1",
        timestamp: Date.now(),
      })),
    });
    ...
    assert.equal(body.macDeviceId, "mac-device-1");
    assert.equal(body.macIdentityPublicKey, "mac-public-key-device-1");
    assert.equal(body.displayName, "Device-Mac");
    assert.equal(body.connectionMode, "persistent_device_channel");
    assert.equal(body.sessionId, "live-device-1");
    assert.equal(body.resumeKey, "live-device-1");
    assert.match(body.webSocketURL, /^ws:\/\/127\.0\.0\.1:\d+\/relay\/device~/);

    const iphone = new WebSocket(body.webSocketURL, {
    ...
    mac.send(JSON.stringify({ ok: "device-channel" }));
    assert.equal(await received, "{\"ok\":\"device-channel\"}");

    const iphoneClosed = onceClosed(iphone);
    iphone.close();
    await iphoneClosed;

    const iphoneRetry = new WebSocket(body.webSocketURL, {
      headers: { "x-role": "iphone" },
    });
    await onceOpen(iphoneRetry);

    const retried = new Promise((resolve) => {
      iphoneRetry.once("message", (value) => resolve(value.toString("utf8")));
    });
    mac.send(JSON.stringify({ ok: "device-channel-retry" }));
    assert.equal(await retried, "{\"ok\":\"device-channel-retry\"}");

    const macClosed = onceClosed(mac);
    const iphoneRetryClosed = onceClosed(iphoneRetry);
    mac.close();
    iphoneRetry.close();
    await Promise.all([macClosed, iphoneRetryClosed]);
  });
});
