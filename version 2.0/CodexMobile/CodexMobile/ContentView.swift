...
struct ContentView: View {
    ...
    private func attemptSavedMacReconnectRecoveryIfNeeded() async {
        let allowsBackgroundGraceRecovery = scenePhase == .background && codex.canAttemptBackgroundGraceReconnect
        guard scenePhase == .active || allowsBackgroundGraceRecovery,
              hasSeenOnboarding,
              !isShowingManualScanner,
        ...
        guard scenePhase == .active,
            ...
            debugSidebarLog(
                "prewarm skipped phase=\(String(describing: scenePhase)) onboarding=\(hasSeenOnboarding) "
                    + "scanner=\(isShowingManualScanner) prewarmed=\(isSidebarPrewarmed) "
                    + "taskActive=\(sidebarPrewarmTask != nil) "
