...
let codexDeviceChannelTokenPrefix = "device~"
...
struct CodexTrustedMacRecord: Codable, Sendable {
    ...
    var lastResolvedResumeKey: String? = nil
...
struct CodexTrustedSessionResolveRequest: Codable, Sendable {
    ...
    let requestedConnectionMode: String?
}
...
struct CodexTrustedSessionResolveResponse: Codable, Sendable {
    ...
    let sessionId: String
    let webSocketURL: String? = nil
    let connectionMode: String? = nil
...
extension CodexTrustedSessionResolveResponse {
    var hasEphemeralDeviceChannelTarget: Bool {
        sessionId.trimmingCharacters(in: .whitespacesAndNewlines)
            .hasPrefix(codexDeviceChannelTokenPrefix)
    }

    var replayCursorIdentity: String {
        if let resumeKey,
           !resumeKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return "resume:\(resumeKey.trimmingCharacters(in: .whitespacesAndNewlines))"
        }

        return connectionTargetKey
    }

    func resolvedWebSocketURL(fallbackRelayURL: String?) -> String? {
        if let webSocketURL,
           !webSocketURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return webSocketURL.trimmingCharacters(in: .whitespacesAndNewlines)
        }

        guard let fallbackRelayURL,
              !fallbackRelayURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return nil
        }

        return "\(fallbackRelayURL.trimmingCharacters(in: .whitespacesAndNewlines))/\(sessionId)"
    }

    var connectionTargetKey: String {
        if let webSocketURL,
           !webSocketURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return "ws:\(webSocketURL.trimmingCharacters(in: .whitespacesAndNewlines))"
        }
