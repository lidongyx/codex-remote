...
extension CodexService {
    ...
    func applyResolvedTrustedSession(_ resolved: CodexTrustedSessionResolveResponse, relayURL: String) {
        let previousReplayIdentity = replayCursorIdentity(
            for: resolved.macDeviceId,
            fallbackRelayURL: relayURL
        )
        let shouldResetReplayCursor = previousReplayIdentity.isEmpty
            || previousReplayIdentity != resolved.replayCursorIdentity
...
private extension CodexService {
    ...
    func resolveTrustedMacSessionImpl() async throws -> CodexTrustedSessionResolveResponse {
        ...
        guard let deviceConnectURL = trustedDeviceConnectURL(from: relayURL) else {
        ...
        let requestBody = CodexTrustedSessionResolveRequest(
            ...
            timestamp: timestamp,
            signature: signature,
            requestedConnectionMode: "persistent_device_channel"
        )

        do {
            return try await resolveTrustedMacSession(
                using: deviceConnectURL,
                relayURL: relayURL,
                requestBody: requestBody,
                unsupportedCode: "device_not_registered"
            )
        } catch let error as CodexTrustedSessionResolveError {
            guard case .unsupportedRelay = error,
                  let legacyResolveURL = trustedSessionResolveURL(from: relayURL) else {
                throw error
            }

            return try await resolveTrustedMacSession(
                using: legacyResolveURL,
                relayURL: relayURL,
                requestBody: requestBody,
                unsupportedCode: nil
            )
    ...
    private func resolveTrustedMacSession(
        using resolveURL: URL,
        relayURL: String,
        requestBody: CodexTrustedSessionResolveRequest,
        ...
        if (200..<300).contains(httpResponse.statusCode) {
            guard let resolved = try? JSONDecoder().decode(CodexTrustedSessionResolveResponse.self, from: data),
                  resolved.ok,
                  !resolved.hasEphemeralDeviceChannelTarget,
                  resolved.resolvedWebSocketURL(fallbackRelayURL: relayURL) != nil else {
        ...
        let errorResponse = try? JSONDecoder().decode(CodexRelayErrorResponse.self, from: data)
        switch errorResponse?.code {
        case "session_unavailable", "device_offline":
            secureConnectionState = .liveSessionUnresolved
        ...
        default:
            if httpResponse.statusCode == 404 || errorResponse?.code == unsupportedCode {
    ...
    private func rememberResolvedTrustedSession(_ resolved: CodexTrustedSessionResolveResponse, relayURL: String) {
        ...
        if var trustedMac = trustedMacRegistry.records[resolved.macDeviceId] {
            ...
            trustedMac.lastResolvedResumeKey = resolved.resumeKey
    ...
    private func replayCursorIdentity(for macDeviceId: String, fallbackRelayURL: String) -> String {
        if let trustedMac = trustedMacRegistry.records[macDeviceId],
           let lastResolvedResumeKey = trustedMac.lastResolvedResumeKey?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !lastResolvedResumeKey.isEmpty {
            return "resume:\(lastResolvedResumeKey)"
        }

        if let currentRelayURL = normalizedRelayURL,
           let currentSessionId = normalizedRelaySessionId {
            return "session:\(currentRelayURL)/\(currentSessionId)"
        }

        if let fallbackSessionId = relaySessionId?.trimmingCharacters(in: .whitespacesAndNewlines),
           !fallbackSessionId.isEmpty {
            return "session:\(fallbackRelayURL)/\(fallbackSessionId)"
        }

        return ""
    }

    private func trustedDeviceConnectURL(from relayURL: String) -> URL? {
        ...
        if pathComponents.last == "relay" {
            ...
            components.path = "/" + (prefix + ["v1", "trusted", "device", "connect"]).joined(separator: "/")
        } else {
            components.path = "/v1/trusted/device/connect"
        }
