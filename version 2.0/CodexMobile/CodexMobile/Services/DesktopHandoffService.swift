...
extension DesktopHandoffError {
    // Best-effort wake pulses should quietly fall back to reconnect UI when the
    // bridge cannot execute the local display-wake command.
    var shouldSuppressConnectionRecoveryMessage: Bool {
        switch self {
        case .bridgeError(let code, _):
            return code == "wake_display_failed"
        default:
            return false
        }
    }
