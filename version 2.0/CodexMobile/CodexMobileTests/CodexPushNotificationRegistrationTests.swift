...
// Purpose: Verifies local notification permission refresh and deferred notification tap routing.
// Layer: Unit Test
...
final class CodexPushNotificationRegistrationTests: XCTestCase {
    ...
    func testRequestNotificationPermissionRefreshesAuthorizedStatus() async {
        let center = MockUserNotificationCenter(status: .authorized)
        let service = makeService(userNotificationCenter: center)
    ...
    func testRefreshManagedNotificationRegistrationStateTracksSettingsChanges() async {
        let center = MockUserNotificationCenter(status: .denied)
        let service = makeService(userNotificationCenter: center)

        await service.refreshManagedNotificationRegistrationState()
        XCTAssertEqual(service.notificationAuthorizationStatus, .denied)
    ...
    private func makeService(userNotificationCenter: CodexUserNotificationCentering) -> CodexService {
