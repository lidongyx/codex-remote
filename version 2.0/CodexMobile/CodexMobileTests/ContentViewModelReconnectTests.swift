...
final class ContentViewModelReconnectTests: XCTestCase {
    ...
    func testBackgroundGraceReconnectMakesOnlyOneRetryableAttempt() async {
        let service = makeService()
        let viewModel = ContentViewModel()
        var attempts = 0
        ...
        service.backgroundTurnGraceTaskID = UIBackgroundTaskIdentifier(rawValue: 1)
        ...
        await viewModel.attemptAutoReconnectOnForegroundIfNeeded(
            codex: service,
            allowBackgroundGrace: true
        )

        XCTAssertEqual(attempts, 1)
        XCTAssertTrue(service.shouldAutoReconnectOnForeground)
        XCTAssertNil(service.lastErrorMessage)
        XCTAssertEqual(service.connectionRecoveryState, .retrying(attempt: 1, message: "Reconnecting..."))
    }
    ...
    func testPreferredReconnectURLUsesResolvedDeviceChannelURLWhenAvailable() async {
        let service = makeService()
        let viewModel = ContentViewModel()
        let macDeviceID = "mac-\(UUID().uuidString)"
        let relayURL = "wss://relay.local/relay"
        ...
        service.trustedSessionResolverOverride = {
            CodexTrustedSessionResolveResponse(
                ...
                webSocketURL: "wss://relay.local/device-channels/mac-\(macDeviceID)",
        ...
        XCTAssertEqual(
            reconnectURL,
            "wss://relay.local/device-channels/mac-\(macDeviceID)"
        )
    }
