...
This repo contains the local bridge, the self-hostable relay, the iOS app target, and their tests:

```
├── phodex-bridge/                # Node.js bridge package used by `remodex`
│   ├── bin/                      # CLI entrypoints
│   └── src/                      # Bridge runtime, git/workspace handlers, refresh helpers
├── relay/                        # Node.js relay server and push session helpers
│   ├── server.js                 # Relay entrypoint
│   └── relay.js                  # Session coordination and relay logic
...
- `bridge-dev`: bridge-focused work without mixing relay or iOS experiments
- `relay-dev`: relay-focused work and self-hosting changes
